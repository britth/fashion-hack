...
post '/' do #was /hello/
  API_BASE = "http://api.shopstyle.com/api/v2/products/histogram?pid=#{ENV["SHOPSENSE_KEY"]}&filters=Retailer&fts="
    ...
    color = all_items["products"].compact.first["colors"].first["name"]
    #colors = selection["products"].first["colors"].map{|x| x["name"]}
    colors = all_items["products"].compact.map{|x| x["colors"].compact.first["name"]}.select{|x| x != color}.uniq{|x| x}
    ...
    sizes = all_items["products"].compact.map{|x| x["sizes"].compact.first["canonicalSize"]["name"]}.uniq{|x| x}.sort_by{|x| x}
    data = {
